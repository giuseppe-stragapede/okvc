...
def compute_sir(scores, comp_type):
    ...
    for k in list(scores_by_demo_group.keys()):
        scores_by_demo_group_.append(np.concatenate((scores_by_demo_group[(k[0], k[1])], scores_by_demo_group[(k[1], k[0])])))
    mean_scores_by_demo_group = np.reshape(np.array([np.mean(x) for x in scores_by_demo_group_]), (len(np.unique(categs[:, 0])), len(np.unique(categs[:, 0]))))
...
def plot_hist(ax, labels, scores, name_saved='score_distributions', displayed_name='My System', location=''):
    ...
    ax.hist(genuine_scores, bins=int(len(genuine_scores)/100), histtype='step', label='Genuine Distribution', color='green')
    ax.hist(impostor_scores, bins=int(len(impostor_scores)/100), histtype='step', label='Impostor Distribution', color='red')
...
def plot_sir_heat_maps(ax, imp_scores, label_names, sir_value, name_saved='sir_heat_map', displayed_name='My System', location=''):
    """
    :param ax: matplotlib Axes object
